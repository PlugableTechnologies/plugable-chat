...
async fn test_format_compliance_hermes() {
    ...
    let settings = ToolCapabilityTestHarness::create_test_settings(false, false, ToolCallFormatName::Hermes);
...
async fn test_format_compliance_native() {
    ...
    let settings = ToolCapabilityTestHarness::create_test_settings(false, false, ToolCallFormatName::Native);
    let model_info = ToolCapabilityTestHarness::create_test_model_info(true, ToolFormat::OpenAI);
    let filter = ToolLaunchFilter::default();
    let registry = ToolCapabilityTestHarness::create_test_registry();
    let server_configs = vec![];
...
async fn test_tool_search_discovers_deferred() {
    ...
    let settings = ToolCapabilityTestHarness::create_test_settings(false, true, ToolCallFormatName::Hermes);
    let model_info = ToolCapabilityTestHarness::create_test_model_info(false, ToolFormat::Hermes);
    let filter = ToolLaunchFilter::default();
    let mut registry = ToolCapabilityTestHarness::create_test_registry();
    let server_configs = vec![];
...
async fn test_native_fallback_to_hermes() {
    ...
    let settings = ToolCapabilityTestHarness::create_test_settings(false, false, ToolCallFormatName::Native);
    let model_info = ToolCapabilityTestHarness::create_test_model_info(false, ToolFormat::Hermes); // Model doesn't support native
    let filter = ToolLaunchFilter::default();
    let registry = ToolCapabilityTestHarness::create_test_registry();
    let server_configs = vec![];
