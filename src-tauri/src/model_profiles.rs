...
impl ModelProfile {
    ...
    fn build_tool_system_prompt_openai(
        &self,
        tools: &[ToolSchema],
        options: &PromptOptions,
    ) -> String {
        ...
        let mut prompt = String::from(
            "You are a tool-using assistant. Use available tools when appropriate.\n\n",
        );
...
"#.to_string()
    ...
    fn build_tool_system_prompt_hermes(
        &self,
        tools: &[ToolSchema],
        options: &PromptOptions,
    ) -> String {
        ...
        let mut prompt =
            String::from("You are a helpful assistant with tool-calling capabilities.\n\n");
    ...
    fn build_tool_system_prompt_granite(
        &self,
        tools: &[ToolSchema],
        options: &PromptOptions,
    ) -> String {
        ...
        let mut prompt =
            String::from("You are a helpful assistant with function calling capabilities.\n\n");
    ...
    fn build_tool_system_prompt_text_based(
        &self,
        tools: &[ToolSchema],
        options: &PromptOptions,
    ) -> String {
    ...
    fn python_only_tool_prompt(tools: &[ToolSchema]) -> String {
        ...
        prompt.push_str(
            "You must return exactly one runnable Python program when tools are available.\n\n",
        );
...
pub fn resolve_profile(model_name: &str) -> &'static ModelProfile {
    for profile in PROFILES.iter() {
        if profile.matches(model_name) {
            println!(
                "[ModelProfiles] Resolved '{}' to profile '{}'",
