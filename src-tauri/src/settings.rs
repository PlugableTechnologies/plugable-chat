...
pub fn ensure_demo_tools_yaml() -> Option<std::path::PathBuf> {
    ...
    let db_path_str = demo_db_path.to_string_lossy().replace('\\', "/");
    let content = format!(
...
pub fn default_demo_database_source() -> DatabaseSourceConfig {
    ...
    let tools_file_path = ensure_demo_tools_yaml()
        ...
        .unwrap_or_else(|| {
            ...
            find_test_data_dir()
                .map(|p| p.join("demo-tools.yaml").to_string_lossy().to_string())
    ...
    DatabaseSourceConfig {
        ...
        transport: Transport::Stdio,
        command: None, // User must set path to toolbox binary (e.g., /opt/homebrew/bin/toolbox)
        args: vec![
