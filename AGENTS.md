...
- **Formatted Results Visible**: When tool calls return structured data (e.g., SQL query results), display the formatted/parsed result prominently in the main chat area (not hidden inside an accordion). The accordion should contain raw request/response data for debugging.
